...
export default function Index() {
    ...
    const fetchtrailersNetflixJSON = async () => {
        const response = await fetch('https://trailersapi.herokuapp.com/api/starplus');
    ...
    <S.Container>
        ...
        <S.SearchArea>
           ...
           {/* <CssTextField label="Custom CSS" id="custom-css-outlined-input" /> */}
            <TextField fullWidth color='secondary' id="custom-css-outlined-input" value={valueSearch} onChange={(e)=>setValueSearch(e.target.value)} label="Pesquisar"
        ...
        <S.Trailers>
            ...
            <S.ModalArea onLoad={()=>setLoadingModal(false)} style={{visibility: hiddeModal ? 'visible' : 'hidden'}}>   
                <S.Modal style={{transition: 'all ease-in-out .3s', top: hiddeModal ? '35px' : '100vh'}}>
                    ...
                    <S.VideoModel frameBorder="0" src={`${valueModal?.url_video}${hiddeModal ? '?autoplay=1' : '?autoplay=0'}${hiddeModal ? '' : '&pause=1'}`} title={valueModal?.name} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" ></S.VideoModel>
                    <S.InfoModal>
